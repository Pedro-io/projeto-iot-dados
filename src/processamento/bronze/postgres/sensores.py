...
class SensoresETL(AbstractETL):
    """ETL para extração da tabela mestre `sensores` do PostgreSQL.

    Carga completa (full load) - tabela com dados cadastrais dos sensores
    instalados nos equipamentos, incluindo tipo de medição e status operacional.
    """
    ...
    def __init__(
        ...
        super().__init__(
            spark=spark,
            table_path="bronze/postgres/sensores",
    ...
    def extract(self) -> None:
        """Extrai todos os registros de sensores."""
        logger.info("Extraindo dados de erp_legado.sensores")
        self.df = self._jdbc_read("erp_legado.sensores")
    ...
    def unit_tests(self) -> None:
        """Verifica completude e validade do status dos sensores."""
        self.error_checks = (
            self.ErrorCheck
            .isComplete("id")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ETL Bronze - PostgreSQL Sensores")
    ...
    spark = (
        ...
        .appName("bronze-postgres-sensores")
    ...
    logger.info("Iniciando pipeline Bronze - PostgreSQL Sensores")
